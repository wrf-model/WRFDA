...
ncmpi_put_vara_all(int ncid, int varid,
  ...
  /* set the mpi file view */
fprintf(stderr,"%s %d calling set_vara_fileview\n",__FILE__,__LINE__) ; 
  status = set_vara_fileview(ncp, &(ncp->nciop->collective_fh), varp, start, count, 0);
fprintf(stderr,"%s %d back from set_vara_fileview\n",__FILE__,__LINE__) ; 
  if(status != NC_NOERR)
    return status;
...
fprintf(stderr,"%s %d calling MPI_File_write_all\n",__FILE__,__LINE__) ; 
  mpireturn = MPI_File_write_all(ncp->nciop->collective_fh, xbuf, nbytes, MPI_BYTE, &mpistatus);
fprintf(stderr,"%s %d back from MPI_File_write_all mpireturn=%d\n",__FILE__,__LINE__,mpireturn) ; 
  if (mpireturn != MPI_SUCCESS) {
        char errorString[512];
        int  errorStringLen;
        MPI_Error_string(mpireturn, errorString, &errorStringLen);
