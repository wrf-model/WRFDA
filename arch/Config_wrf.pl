...
until ( $validresponse ) {
  ...
  $opt = 1 ;
  open CONFIGURE_DEFAULTS, "< ./arch/configure.defaults_wrf" 
      or die "Cannot open ./arch/configure.defaults_wrf for reading" ;
...
while ( <CONFIGURE_DEFAULTS> )
{
  ...
  if ( $latchon == 1 )
  {
    ...
    # ESMF substitutions in configure.defaults_wrf
...
#printf "     arch/configure.defaults_wrf\n" ;
#printf "\n" ;

open CONFIGURE_WRF, "> configure.wrf" or die "cannot append configure.wrf" ;
open ARCH_PREAMBLE, "< arch/preamble_wrf" or die "cannot open arch/preamble_wrf" ;
my @preamble;
...
open ARCH_POSTAMBLE, "< arch/postamble_wrf" or die "cannot open arch/postamble_wrf" ;
