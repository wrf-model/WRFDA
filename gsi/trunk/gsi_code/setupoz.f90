...
! !USES:
  ...
  use constants, only : rozcon,cg_term,wgtlim,r3600

  use obsmod, only:  ozhead,oztail,ozohead,ozotail,dplat
...
!    Process obs have at least one piece of information that passed qc checks
     if (.not. last .and. ikeep==1) then
        ...
        allocate(oztail%res(nlev),oztail%err2(nlev), &
                 oztail%raterr2(nlev),oztail%prs(nlev-1), &
                 oztail%wij(4,nsig), &
...
!    Process obs have at least one piece of information that passed qc checks
     if ( .not. last .and. ikeep==1) then
        ...
        allocate( ozotail%wij(4,nsig),stat=istatus)
        if (istatus/=0) write(6,*)'SETUPOZ:  allocate error for ozo_point, istatus=',istatus

!       Set number of levels for this obs
        ozotail%nloz = 1

!       Set (i,j) indices of guess gridpoint that bound obs location
        call get_ij(mm1,dlat,dlon,ozotail%ij(1),tempwij(1))

        call tintrp2a(ges_prsi,prsitmp,dlat,dlon,dtime,hrdifsig,&
             1,nsig+1,mype,nfldsig)

        do k = 1,nsig
          ozotail%wij(1,k)=tempwij(1)*rozcon*(prsitmp(k)-prsitmp(k+1))
          ozotail%wij(2,k)=tempwij(2)*rozcon*(prsitmp(k)-prsitmp(k+1))
          ozotail%wij(3,k)=tempwij(3)*rozcon*(prsitmp(k)-prsitmp(k+1))
          ozotail%wij(4,k)=tempwij(4)*rozcon*(prsitmp(k)-prsitmp(k+1))
        end do
