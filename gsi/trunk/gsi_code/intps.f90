...
! Declare local variables
  integer(i_kind) i,j1,j2,j3,j4
! real(r_kind) penalty
  real(r_kind) cg_ps,val,p0,grad,wnotgross,wgross,term,ps_pg,varqc_iter
...
!    variational qc is gradually increased to avoid possible convergence problems
     ...
     if (nlnqc_iter .and. psptr%pg > tiny_r_kind .and. psptr%b >tiny_r_kind) then
        ...
        wnotgross= one-ps_pg                            ! pg is A in the reference by Enderson
        wgross =ps_pg*cg_ps/wnotgross                   ! wgross is gama in the reference by Enderson
