...
!   2007-07-03  add variance reweighting option
!   2008-08-25  hu - add variable and function for controling 
!                    cloud analysis and initialization
!                  - add and read namelist for cloud analysis
!
! usage:
!   input files:
! **************************
...
!          =  110- problem with reading METAR cloud observation 
!          =  111- problem with reading radar mosaic reflectivity 
!          =  112- problem with reading lightning data 
!          =  113- problem in general cloud analysis driver 
!          =  114- problem in general cloud analysis package 
!
...
!==================================================================================================
  ...
  use obsmod, only: dmesh,dval,dthin,dtype,dfile,dplat,ndat,diag_conv,&
     ...
     dtbduv_on,time_window_max,offtime_data,init_directories
  ...
  use convinfo, only: init_convinfo,nconvtype,npred_conv_max, &
  ...
  use turbmod, only: use_pbl,init_turb
  ...
  use berror, only: norh,ndeg,vs,as,bw,init_berror,hzscl,hswgt,pert_berr,pert_berr_fct,&
     bkgv_flowdep,bkgv_rewgtfct,tsfc_sdv
...
#endif
  use cloudanalysismod, only: cloud_analysis,init_cloudanalysis
  implicit none
...
!            add use of guess file later for regional mode.
  ...
  namelist/setup/gencode,factqmin,factqmax,deltim,dtphys,biascor, &
       ...
       iout_iter,npredp,retrieval,jpch,jtype,&
       diag_rad,diag_pcp,diag_conv,diag_ozone,iguess,write_diag,&
       oneobtest,obs_sen,switch_on_derivatives,tendsflag,&
...
!     perturb_obs - logical flag to perutrb observation or not
!     c_varqc - constant number to control var. qc turnning on speed
!     perturb_fact - the magnitude fact of observation perturbation

  namelist/obsqc/ repe_dw,repe_gps,dfact,dfact1,erradar_inflate,oberrflg,vadfile,noiqc,&
       perturb_obs,perturb_fact,c_varqc
...
!      cloud_analysis     -   if .true., cloud analysis on
  namelist/cloudanalysis/cloud_analysis
