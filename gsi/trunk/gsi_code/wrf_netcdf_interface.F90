...
!   2008-06-02  Hu  - add code to read 5 cloud/hydrometeor variables for cloud analysis
...
!$$$
  ...
  use cloudanalysismod, only: cloud_analysis
  implicit none
...
!   2008-06-02  Hu  - add code to update 5 cloud/hydrometeor variables for cloud analysis
!   2008-06-27  Hu  - bug fix: replace XICE with SEAICE and comment out update for SMOIS (the actually 
...
! Declare local parameters
  ...
  character (len= 4) :: staggering=' N/A'
  character (len= 3) :: ordering,ordering1,ordering2, ord
  character (len=24) :: start_date1,start_date2
  character (len=24) :: current_date,current_date1,current_date2
  character (len=31) :: name,name1,name2,name3,name4,name5
  character (len=25) :: units,        units2
  character (len=46) :: description,  description2
...
!        nlon_regional,nlat_regional,nsig_regional,pt_regional
  ...
  call ext_ncd_write_field(dh1,DateStr1,TRIM(rmse_var),              &
       ...
       ierr                                 )
!  
!  by mhu: actually, the following 2d field is Landmask, not SMOIS (soil moisture)
!     so, commnet out all
!
