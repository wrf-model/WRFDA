...
!   2006-07-31  kleist - change to use ges_ps instead of lnps
!   2007-03-13  derber - remove unused qsinv2 from jfunc use list
!   2008-08-25  hu     - add code to read in cloud/hydrometeor fields and distributed them to all processors 
!
!   input argument list:
!     mype     - pe number
!
...
!$$$
  ...
  use guess_grids, only: ges_z,ges_ps,ges_tv,ges_q,ges_u,ges_v,&
       fact10,soil_type,veg_frac,veg_type,sfct,sno,soil_temp,soil_moi,&
       isli,ntguessig,nfldsig,ifilesig,ges_tsen,                      &
       ges_qc,ges_qi,ges_qr,ges_qs,ges_qh,ges_xlon,ges_xlat,ges_tten
  use gridmod, only: lat2,lon2,nlat_regional,nlon_regional,&
...
! other internal variables
  ...
  integer(i_kind) i_qc,i_qi,i_qr,i_qs,i_qh,kqc,kqi,kqr,kqs,kqh, i_xlon,i_xlat,i_tt,ktt
  integer(i_kind) i_sm,i_xice,i_sst,i_tsk,i_ivgtyp,i_isltyp,i_vegfrac
  integer(i_kind) isli_this
  real(r_kind) psfc_this,psfc_this_dry,sm_this,xice_this
  real(r_kind),dimension(lat2,lon2):: q_integral
...
!    Following is for convenient WRF MASS input
     ...
     if(cloud_analysis) num_mass_fields=14+4*lm + 6*lm  + 2 
...
! for cloud analysis
     ...
     if(cloud_analysis) jsig_skip(i)=0     ! number of files to skip before getting to psfc
     igtype(i)=1
...
! cloud 
                 if(cloud_analysis) then
                   ...
                   ges_qh(j,i,k,it) = all_loc(j,i,kqh)
