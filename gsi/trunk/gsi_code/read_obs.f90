...
!   2008-08-25  hu      - add cloud observations options in prepbufr
!                         and add calls for reading in lightning and NSSL mosaic
!
!   input argument list:
!     mype     - mpi task id
!
...
!   type type of GPS data (if present)
    ...
    do i=1,ndat
       ...
       if (obstype == 't'  .or. obstype == 'uv' .or. &
           ...
           obstype == 'rad_ref' .or. obstype=='lghtn')  then
           ditype(i) = 'conv'
...
!    Process NSSL reflectivity Mosaic
             else if (obstype == 'rad_ref' ) then
                call read_NSSL_mosaic(nread,npuse,infile,obstype,lunout)
                   string='READ_NSSL_MOSAIC'

!    Process  lightning
             else if (obstype == 'lghtn' ) then
                call read_lightning(nread,npuse,infile,obstype,lunout,mype)
