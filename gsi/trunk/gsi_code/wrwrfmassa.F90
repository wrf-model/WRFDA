...
!   2006-07-28  derber  - include sensible temperature
!   2006-07-31  kleist - change to use ges_ps instead of lnps
!   2008-08-25  hu     - add code to gether cloud/hydrometeor fields and write out
!
!   input argument list:
!     mype     - pe number
!
...
!$$$
  ...
  use guess_grids, only: ntguessfc,ntguessig,ifilesig,sfct,ges_ps,&
       ...
       ges_qc,ges_qi,ges_qr,ges_qs,ges_qh,ges_tten
...
! Declare local variables
  ...
  integer(i_kind) i_qc,i_qi,i_qr,i_qs,i_qh,kqc,kqi,kqr,kqs,kqh,i_tt,ktt
  ...
  if(cloud_analysis) num_mass_fields=3+4*lm + 6*lm
...
! for cloud             
           if(cloud_analysis) then
             ...
             all_loc(j,i,kqh)=ges_qh(j,i,k,it)
...
! Update qh
  ...
  do k=1,nsig
     kqh=kqh+1
     if(mype == 0) read(lendian_in)temp1
     call strip_single(all_loc(1,1,kqh),strp,1)
     call mpi_gatherv(strp,ijn(mype+1),mpi_real4, &
          tempa,ijn,displs_g,mpi_real4,0,mpi_comm_world,ierror)
     if(mype == 0) then
        call fill_mass_grid2t(temp1,im,jm,tempb,2)
...
! done with update cloud and hydrometeor fields
