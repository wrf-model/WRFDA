...
!$$$
  ...
  integer(i_kind) nbd
  real(r_single) bdfilter(5),bdcoef
...
! Mass grids--just copy
  do j=1,ny
     do i=1,nx
       bdcoef = 1.0
       if(i <= nbd .or. j<=nbd ) then
        bdcoef=bdfilter(min(i,j))
        gin(i,j)=b(i,j)*bdcoef + gin(i,j)
       elseif(i >= nx-nbd+1 .or. j>=ny - nbd + 1) then
        bdcoef=bdfilter(min(nx-i+1,ny-j+1))
        gin(i,j)=b(i,j)*bdcoef + gin(i,j) 
       else
...
!$$$
  ...
  do j=1,ny
     do i=1,nx+1
       ...
       if(i <= nbd .or. j<=nbd ) then
        bdcoef=bdfilter(min(i,j))
        gin(i,j)=gin(i,j)+half*(b(im,j)+b(i0,j)) * bdcoef
       elseif(i >= nx+1-nbd+1 .or. j>=ny - nbd + 1) then
        bdcoef=bdfilter(min(nx+1-i+1,ny-j+1))
        gin(i,j)=gin(i,j)+half*(b(im,j)+b(i0,j)) * bdcoef
       else
...
!$$$
  ...
  do j=1,ny+1
     ...
     do i=1,nx
       ...
       if(i <= nbd .or. j<=nbd ) then
        bdcoef=bdfilter(min(i,j))
        gin(i,j)=gin(i,j)+half*(b(i,jm)+b(i,j0)) * bdcoef
       elseif(i >= nx-nbd+1 .or. j>=ny+1 - nbd + 1) then
        bdcoef=bdfilter(min(nx-i+1,ny+1-j+1))
        gin(i,j)=gin(i,j)+half*(b(i,jm)+b(i,j0)) * bdcoef
       else
