...
!   2008-08-25  hu     - add special treat for mta_cld
!
!   input argument list:
...
!    ibw,ibe,ibs,ibn west,east,south and north boundaries of total region
  ...
  isprd=0
  if(obstype.eq.'mta_cld' ) isprd=10
...
!       Does the observation belong to the subdomain for this task?
     if(lat>=(ibs(mm1)-isprd).and.lat<=(ibn(mm1)+isprd)) then
        if((lon >= (ibw(mm1)-isprd).and. lon <=(ibe(mm1)+isprd))  .or.  &
...
!              Does the observation belong to the subdomain for other tasks?
               if(lusenew)then
                  if(lat>=(ibs(k)-isprd).and.lat<=(ibn(k)+isprd)) then
                     if((lon>=  (ibw(k)-isprd).and. lon<=(ibe(k)+isprd))   .or.  &
...
! Write observations for given task to output file
     ...
     deallocate(data1_s,luse,crit,iloc)
  endif
