...
!$$$
    ...
    use gridmod, only: displs_s,irc_s,ltosj_s,ijn_s,ltosi,ltosj,&
         ...
         itotsub,fill_ns,ncep_sigio,ncepgfs_head,idpsfc5,idthrm5,&
...
!   Declare local variables
    integer(i_kind):: iret,nlatm2,ij,n
    ...
    real(r_kind),dimension(nlon,nlat-2):: grid,grid_u,grid_v,&
         grid_vor,grid_div,sum_cp,sum_q
...
!               Convert input thermodynamic variable to dry temperature
                call sigio_cnvtdv8(nlon*(nlat-2),nlon*(nlat-2),1,idvc5,&
...
!
    ...
    use guess_grids, only: ges_z,ges_ps,ges_vor,ges_div,&
         ges_tv,ges_q,ges_oz,ges_cwmr,sfct,fact10,ges_prsl,&
         ges_u,ges_v,ges_prsi
...
!-------------------------------------------------------------------------
    ...
    real(r_kind),dimension(lat1*lon1):: hsm,psm,tsm,sum_cp,sum_q
...
!   Handle three possibilities for SIGIO
    else

!      Only need to take action if requested output type is 
!      T (idthrm5=2) or CpT (idthrm5=3)
       if (idthrm5==2 .or. idthrm5==3) then
...
!   Gather thermodynamic variable from sub-domain to full grid
    do k=1,nsig
       ...
       if (mype==mype_out) then
          ...
          if (ncep_sigio) then
             ...
             call sptez_s(spec_work,grid,-1)
             do i=1,nc
                sigdata%t(i,k)=sigdata%t(i,k)+spec_work(i)
                if(factsml(i))sigdata%t(i,k)=zero_single
             end do
          else
             ij=0
             do j=1,nlatm2
    ...
    do k=1,nsig
       ...
       if (mype==mype_out) then
          ...
          if (ncep_sigio) then
             ...
             call sptez_s(spec_work,grid,-1)
             do i=1,nc
                sigdata%q(i,k,1) =sigdata%q(i,k,1)+spec_work(i)
                if(factsml(i))sigdata%q(i,k,1)=zero_single
             end do
          else
             ij=0
             do j=1,nlatm2
    ...
    do k=1,nsig
       ...
       if (mype==mype_out) then
          ...
          if (ncep_sigio) then
             ...
             call sptez_s(spec_work,grid,-1)
             do i=1,nc
                sigdata%q(i,k,2) =sigdata%q(i,k,2) + spec_work(i)
                if(factsml(i))sigdata%q(i,k,2)=zero_single
             end do
          else
             ij=0
             do j=1,nlatm2
       ...
       do k=1,nsig
          ...
          if (mype==mype_out) then
             ...
             if (ncep_sigio) then
                ...
                call sptez_s(spec_work,grid,-1)
                do i=1,nc
                   sigdata%q(i,k,3) =sigdata%q(i,k,3)+spec_work(i)
                   if(factsml(i))sigdata%q(i,k,3)=zero_single
                end do
             else
                ij=0
                do j=1,nlatm2
...
!      Horizontal divergence and voriticy
       do k=1,nsig
          ...
          if (mype==mype_out) then
             ...
             call sptez_s(spec_work,grid,-1)
             do i=1,nc
                sigdata%d(i,k) = sigdata%d(i,k) + spec_work(i)
                if(factvml(i))sigdata%d(i,k)=zero_single
             end do
          endif
          ...
          if (mype==mype_out) then
             ...
             call sptez_s(spec_work,grid,-1)
             do i=1,nc
                sigdata%z(i,k) =sigdata%z(i,k) + spec_work(i)
                if(factvml(i))sigdata%z(i,k)=zero_single
             end do
          endif
       end do
    endif
