...
! !USES:
  ...
  use obsmod, only: wtail,whead,rmiss_single,perturb_obs,perturb_fact,ran01dom
...
!   2007-03-27  li - add qc for type 289 windsat winds
!
! REMARKS:
!   language: f90
...
! Declare local variables
  ...
  real(r_kind) err_input,err_adjst,err_final,cvar_pg1
...
!    Modify for the obs perturbation to adjust the obs err
     if( muse(i) .and. perturb_obs) then
        if( error >1.0e-2) then
           rand=ran01dom()
           perturbu=rand*perturb_fact/error
           rand=ran01dom()
           perturbv=rand*perturb_fact/error
        else
           perturbu=zero
           perturbv=zero
        endif
      ...
      if( jiter /= jiterstart ) then
          dudiff=dudiff+perturbu
          dvdiff=dvdiff+perturbv
       endif
    endif
...
!    Turn off rainsonde variational qc over south hemisphere
     ...
     if (itype==220 .and. data(ilate,i)<zero) cvar_pg1=zero
...
!    Compute penalty terms (linear & nonlinear qc).
     if(luse(i))then
        ...
        if (cvar_pg1 > tiny_r_kind .and. error > tiny_r_kind) then
           ...
           cg_w=cvar_b(ikx)
           wgross = cg_term*cvar_pg1/(cg_w*wnotgross)
...
!    in inner loop minimization (int* and stp* routines)
     ...
     if (.not. last .and. muse(i)) then
        ...
        wtail%luse=luse(i)
        if(perturb_obs) then
           wtail%upertb=perturbu
           wtail%vpertb=perturbv
           if(presw >= ptabl(2))then
              wtail%wk1=1+ictype(ikx)*100
           else if( presw < ptabl(33)) then
              wtail%wk1=33+ictype(ikx)*100
           else
              k_loop: do k=2,32
                if(presw >= ptabl(k+1) .and. presw < ptabl(k)) then
                   wtail%wk1=k+ictype(ikx)*100
                   exit k_loop
                endif
             enddo k_loop
           endif
