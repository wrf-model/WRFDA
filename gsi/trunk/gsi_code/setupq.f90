...
!$$$
  ...
  use obsmod, only: qtail,qhead,rmiss_single,perturb_obs,perturb_fact,ran01dom
...
!       Set (i,j,k) indices of guess gridpoint that bound obs location
        ...
        qtail%luse   = luse(i)
        if(perturb_obs) then
           qtail%qpertb=perturbb
           if(presq >= ptabl(2))then
              qtail%qk1=1+ictype(ikx)*100
           else if( presq < ptabl(33)) then
              qtail%qk1=33+ictype(ikx)*100
           else
              k_loop: do k=2,32
                if(presq >= ptabl(k+1) .and. presq < ptabl(k)) then
                   qtail%qk1=k+ictype(ikx)*100
                   exit k_loop
                endif
             enddo k_loop
           endif
!           write(6,*) 'SETUPQ:',qtail%qk1,qtail%qpertb,data(8,i),itype,&
!           presq,k,ptabl(k),ptabl(k+1)
        else
           qtail%qpertb=zero
           qtail%qk1=izero
        endif
