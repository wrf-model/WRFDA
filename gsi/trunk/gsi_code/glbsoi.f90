...
!   2008-08-25  hu      - add cloud analysis interface
!
!   input argument list:
!     mype - mpi task id
!
...
!$$$
  ...
  use obsmod, only: write_diag,obs_sen,obs_setup,ndat,perturb_obs,dirname
  use turbmod, only: create_turbvars,destroy_turbvars
...
! Allocate arrays to hold surface and sigma guess fields.
  if(cloud_analysis) call create_cld_grids
...
! Set errors and create variables for dynamical constraint
  ...
  if(cloud_analysis) then
    call GeneralCloudAnalysis(mype)
...
! Deallocate remaining arrays
  ...
  if(cloud_analysis) call destroy_cld_grids
