...
!  determine specific sensor
   ...
   if (sensorindex == 0 ) write(6,*)'SETUPRAD:  problem with sensorindex ',isis
...
!             GFS and NNM as of September 2005
     ...
     surface(1)%water_temperature     = ts5(1)
     surface(1)%land_temperature      = ts5(2)
     surface(1)%ice_temperature       = ts5(3)
     surface(1)%snow_temperature      = ts5(4)
     surface(1)%soil_moisture_content = soilm5
     surface(1)%vegetation_fraction   = vegf5
     surface(1)%soil_temperature      = soilt5
     surface(1)%snow_depth            = snow5
