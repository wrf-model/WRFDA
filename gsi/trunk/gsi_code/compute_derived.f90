...
!   2008-08-08  Devenyi - add vertical coordinate for qoption = 2 and ARW core
!
!   input argument list:
!     mype     - mpi task id
!
...
!$$$
  ...
  use gridmod, only: eta1_ll                          !DEDE 06/02/08
  implicit none
...
! Declare local variables
  ...
  real(r_kind) drh,rd_over_cp_mass_core,d,dl1,dl2,psfc015,dn1,dn2
...
!         sigsum(k)=zero
          sigsum(k)=eta1_ll(k)+eta1_ll(k+1)    !DEDE 06/02/08
       end if
     end do
