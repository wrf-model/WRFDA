...
!----------temporary variables to keep track of number of observations falling in beta_ref jump zone
  real(r_kind) beta_diff_max,beta_diff_min,beta_diff_rms,count_beta_diff,count_beta_diff_gt_20
...
!-------------------------------------------------------------------------
    ...
    if (allocated(cp5)) deallocate(cp5)
        if(count_beta_diff.gt.zero.or.count_beta_diff_gt_20.gt.zero) then
          ...
          write(6,*)' BETA_DIFF_MAX,min,rms(deg)=', &
                   beta_diff_max*rad2deg,beta_diff_min*rad2deg,beta_diff_rms*rad2deg
          write(6,*)' count_beta_diff,count_beta_diff_gt_20=',count_beta_diff,count_beta_diff_gt_20
          write(6,*)' beta_diff_max_gt_20(deg)=',beta_diff_max_gt_20*rad2deg
        end if
    return
...
!  interpolate departure from longitude part of angle between earth positive east and local positive x
  ...
  if(thisdiff*rad2deg.gt.20._r_kind) then
