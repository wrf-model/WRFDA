...
!  for cloud analysis:
  ...
  real(r_kind),allocatable,dimension(:,:,:,:):: ges_qr    ! cloud rain
  real(r_kind),allocatable,dimension(:,:,:,:):: ges_qs    ! cloud snow
  real(r_kind),allocatable,dimension(:,:,:,:):: ges_qh    ! cloud hail

  real(r_kind),allocatable,dimension(:,:,:):: ges_xlon    ! longitude
  real(r_kind),allocatable,dimension(:,:,:):: ges_xlat    ! latitude
! for temperature tendency
  real(r_kind),allocatable,dimension(:,:,:,:):: ges_tten    ! temperature tendency
...
!   Allocate and zero guess grids
    allocate (ges_qc(lat2,lon2,nsig,nfldsig),&
         ...
         ges_qs(lat2,lon2,nsig,nfldsig),ges_qh(lat2,lon2,nsig,nfldsig),&
...
!-------------------------------------------------------------------------
    ...
    deallocate (ges_qc, ges_qi,ges_qr,ges_qs,ges_qh,ges_xlon,ges_xlat,ges_tten,stat=istatus)
    if (istatus/=0) &
