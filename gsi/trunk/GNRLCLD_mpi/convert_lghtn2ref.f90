SUBROUTINE convert_lghtn2ref(mype,nlon,nlat,nsig,ref_mos_3d,lightning,g3_2)
!
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:  convert_lghtn2ref  convert lightning strokes to radar reflectivity
!
!   PRGMMR: Ming Hu          ORG: GSD/AMB        DATE: 2008-11-17
!
! ABSTRACT: 
!  This subroutine convert lightning strokes to radar reflectivity
!
! PROGRAM HISTORY LOG:
...
!
  ...
  real,intent(in)   :: lightning(nlon,nlat)
  real :: dbz_lightning(nlon,nlat)
  real :: table_lghtn2ref_winter(30)   ! table content the map from lightning strakes 
  ...
  real :: table_lghtn2ref_summer(30)   ! table content the map from lightning strakes 
                                       ! to maximum reflectivity 
  ...
  real :: refprofile_winter(maxlvl,4)   ! stattic reflectivity profile used to retrieve 
...
!  max reflectivity 45-50 dbz
  ...
  real :: refprofile_summer(maxlvl,4)   ! stattic reflectivity profile used to retrieve                                  ! vertical ref based on lightning
!  max reflectivity 30-35 dbz
...
!  max reflectivity 45-50 dbz
  ...
  INTEGER :: num_lightning
  INTEGER :: i,j, k2, k, mref
  REAL :: heightGSI,upref,downref,wght
  INTEGER :: ilvl,numref
  REAL :: lowest,highest,tempref, tempprofile(maxlvl)
...
!
  season=1
  dbz_lightning = -9999.0
  DO j=2,nlat-1
  DO i=2,nlon-1
    if(lightning(i,j) .gt. 0.1 ) then
...
!  ref_mos_3d=-9999.0
  ...
  DO i=2,nlon-1
    if( dbz_lightning(i,j) > 30 ) then
       mref =  min(4,(int((dbz_lightning(i,j) - 30.0)/5.0) + 1 ))
       if(season== 2 ) then
         DO k=1,maxlvl
