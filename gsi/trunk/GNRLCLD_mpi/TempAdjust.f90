...
!
! ABSTRACT: 
!  This subroutine adjust the purterbation potential temperature field to account
    ...
    DO k=2,nsig
      DO j=2,nlat-1
        DO i=2,nlon-1
          ...
          if( arg > 0.0 ) then
            ppi = (p_bk(i,j,k)*p0inv) ** rddcp
            arg = lathv*frac_qc_2_lh*arg*0.001/(cp*ppi)
      ...
      DO j=2,nlat-1
        DO i=2,nlon-1
          IF(w_bk(i,j,k) > 0. .and. ctmp_bk(i,j,k) > 0.0) THEN
            wratio=1.0
            ptcld=ctmp_bk(i,j,k)*(p0/p_bk(i,j,k))**rddcp
