SUBROUTINE build_missing_REFcone(mype,nlon,nlat,nsig,ref_mos_3d,g3_2)
!
!  radar observation
!
!
...
!                .      .    .                                       .
! subprogram:  build_missing_REFcone  build missing reflectivity cone in radar observations
!
!   PRGMMR: Ming Hu          ORG: GSD/AMB        DATE: 2008-11-26
!
! ABSTRACT: 
!  This subroutine build missing reflectivity cone in radar observations
!
! PROGRAM HISTORY LOG:
...
!
  ...
  INTEGER,intent(inout) ::  nlon,nlat,nsig
  real(r_kind),intent(inout) :: ref_mos_3d(nlon,nlat,nsig)            ! reflectivity in grid
  real(r_single),intent(inout) :: g3_2(nlon,nlat,nsig)                  ! height
!
  integer :: krad_bot,ifmissing
  parameter (krad_bot=6)
!
  integer :: maxlvl
  parameter (maxlvl=31)
  real :: newlvlAll(maxlvl)   ! vertical levels of reflectivity statistic profile
  DATA newlvlAll/0.2, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, &
                3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5, &
                9, 10, 11, 12, 13, 14, 15, 16/

  real :: refprofile_winter(maxlvl,6)   ! stattic reflectivity profile used to retrieve 
                                 ! vertical ref based on lightning
...
!  max reflectivity 45-50 dbz
  ...
  real :: refprofile_summer(maxlvl,6)   ! stattic reflectivity profile used to retrieve       
                           ! vertical ref based on lightning
...
!  max reflectivity 45-50 dbz
  ...
  INTEGER :: season   ! 1= summer, 2=winter

  REAL :: heightGSI,upref,downref,wght
  INTEGER :: ilvl,numref
  REAL :: lowest,highest,tempref(nsig), tempprofile(maxlvl)
  REAL :: maxref

  INTEGER :: i,j, k2, k, mref
...
!
  season=2
  DO k=1,maxlvl
     newlvlAll(k)=newlvlAll(k)*1000.0
  ENDDO
!
  DO j=2,nlat-1
  DO i=2,nlon-1
...
!
    DO k2=int(nsig/2),krad_bot,-1
      if(ref_mos_3d(i,j,k2+1).ge.20. .and. ref_mos_3d(i,j,k2).lt.-100. ) ifmissing=k2
      if(ref_mos_3d(i,j,k2).gt.maxref) maxref=ref_mos_3d(i,j,k2)
    ENDDO
    IF(ifmissing > 1 ) then
       DO k2=krad_bot,1,-1
         if(ref_mos_3d(i,j,k2).gt.maxref) maxref=ref_mos_3d(i,j,k2)
    ...
    IF(ifmissing > 1 .and. maxref > 19.0 ) then
       mref =  min(6,(int((maxref - 20.0)/5.0) + 1 ))
       if(season== 2 ) then
         DO k=1,maxlvl
