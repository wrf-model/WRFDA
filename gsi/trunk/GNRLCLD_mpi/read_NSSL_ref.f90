SUBROUTINE read_NSSL_ref(mype,lunin,nlon,nlat,Nmsclvl,numref,ref_mosaic31)
!
!
!
...
!
  ...
  IF( Nmsclvl .ne. 21 .and. Nmsclvl .ne.31) then
     write(6,*) ' read_NSSL_ref: vertical dimesion inconsistent when read in reflectivty mosaic'
  ...
  read(lunin)  ref_in
  DO i=1,numref
    ii=int(ref_in(ilon1s,i)+0.001) - ib + 2
    jj=int(ref_in(ilat1s,i)+0.001) - jb + 2
    if( ii < 1 .or. ii > nlon ) write(6,*) 'read_NSSL_ref: Error in read in ref ii:',mype,ii,jj,i,ib,jb
    if( jj < 1 .or. jj > nlat ) write(6,*) 'read_NSSL_ref: Error in read in ref jj:',mype,ii,jj,i,ib,jb
    DO k=1,Nmsclvl
