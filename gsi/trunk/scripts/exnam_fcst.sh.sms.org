...
NLNAME=nam_namelist.input_fcst_${nltype}

if [ $MODE = fast ]
then
   NLNAME=nam_namelist.input_fcst_${nltype}_fast
fi
...
# Julia: Not sure links to restart/boundary files will work but try it anyway
...
if [ $GUESS = GDAS ] ; then
  ...
  if [ -s $GESDIR/${RUN}.t${cyc}z.wrfrst_anl.${tmmark} ] ; then
     ln -s -f $GESDIR/${RUN}.t${cyc}z.wrfrst_anl.${tmmark} wrfrst_d01_${iy}-${im}-${id}_${ih}:00:00
  fi
fi
