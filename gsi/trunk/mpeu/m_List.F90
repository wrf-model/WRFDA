...
!JFM function indexStr_(aList,itemStr)
!JFM   use m_String,only : String,toChar
!JFM   implicit none
!JFM   type(List),      intent(in) :: aList	! a List of names
!JFM   type(String),    intent(in) :: itemStr
!JFM   integer :: indexStr_

! !REVISION HISTORY:
! 	22Apr98 - Jing Guo <guo@thunder> - initial prototype/prolog/code
!EOP ___________________________________________________________________

!JFM  character(len=*),parameter :: myname_=myname//'::indexStr_'
!JFM  integer :: i,lb,le
!JFM
!JFM  indexStr_=0
!JFM  do i=1,size(aList%lc,2)		! == nitem_(aList)
!JFM    lb=aList%lc(0,i)
!JFM    le=aList%lc(1,i)
!JFM    if(toChar(itemStr)==toChar(aList%bf(lb:le))) then
!JFM      indexStr_=i
!JFM      exit
!JFM    endif
!JFM  enddo
!JFM
!JFMend function indexStr_
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!       NASA/GSFC, Data Assimilation Office, Code 910.3, GEOS/DAS      !
!BOP -------------------------------------------------------------------
!
...
!BOP -------------------------------------------------------------------
!
!JFM !IROUTINE: set_indices_ - set the indices of given items
!JFM
!JFM !DESCRIPTION:
!JFM
!JFM !INTERFACE:
!JFM
!JFM    subroutine set_indices_(indices,aList,values)
!JFM      use m_String, only : String,clean
!JFM      implicit none
!JFM      integer,dimension(:),intent(out) :: indices
!JFM      type(List),intent(in)	  :: aList  ! an indexed string values
!JFM      character(len=*),intent(in) :: Values ! ":" delimited names

! !REVISION HISTORY:
! 	31May98 - Jing Guo <guo@thunder> - initial prototype/prolog/code
!EOP ___________________________________________________________________

!JFM  character(len=*),parameter :: myname_=myname//'::set_indices_'
!JFM  type(List)   :: tList
!JFM  type(String) :: tStr
!JFM  integer :: n,i
!JFM
!JFM  call init_(tList,values)
!JFM  n=min(nitem_(tList),size(indices))
!JFM
!JFM  do i=1,n
!JFM    call get_(tStr,i,tList)
!JFM    indices(i)=indexStr_(aList,tStr)
!JFM    call clean(tStr)
!JFM  end do
!JFM
!JFM  call clean_(tList)
!JFM
!JFMend subroutine set_indices_
