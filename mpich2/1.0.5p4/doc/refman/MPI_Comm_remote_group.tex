\startmanpage
\mantitle{MPI{\tt \char`\_}Comm{\tt \char`\_}remote{\tt \char`\_}group}{tex}{10/14/2006}
\manname{MPI{\tt \char`\_}Comm{\tt \char`\_}remote{\tt \char`\_}group}
...
\startarg{comm }{Communicator (must be an intercommunicator) (handle)
}
\par
\subhead{Output Parameter}
\startarg{group }{remote group of communicator (handle)
}
\par
\subhead{Notes}
...
\nextline
\location{comm{\tt \char`\_}remote{\tt \char`\_}group.c}
\endmanpage
