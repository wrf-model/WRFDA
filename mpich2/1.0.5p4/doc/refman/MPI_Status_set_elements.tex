\startmanpage
\mantitle{MPI{\tt \char`\_}Status{\tt \char`\_}set{\tt \char`\_}elements}{tex}{12/9/2006}
\manname{MPI{\tt \char`\_}Status{\tt \char`\_}set{\tt \char`\_}elements}
...
\startarg{status }{status to associate count with (Status) 
}
\startarg{datatype }{datatype associated with count (handle) 
}
\startarg{count }{number of elements to associate with status (integer) 
}
\par
...
\location{status{\tt \char`\_}set{\tt \char`\_}elements.c}
\endmanpage
