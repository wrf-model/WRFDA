\startmanpage
\mantitle{MPI{\tt \char`\_}Scatter}{tex}{10/24/2006}
\manname{MPI{\tt \char`\_}Scatter}
...
\startarg{sendbuf }{address of send buffer (choice, significant 
only at {\tt root}) 
}
\startarg{sendcount }{number of elements sent to each process 
(integer, significant only at {\tt root}) 
}
\startarg{sendtype }{data type of send buffer elements (significant only at {\tt root}) 
(handle) 
}
