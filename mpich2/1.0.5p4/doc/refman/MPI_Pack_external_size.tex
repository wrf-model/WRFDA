\startmanpage
\mantitle{MPI{\tt \char`\_}Pack{\tt \char`\_}external{\tt \char`\_}size}{tex}{12/9/2006}
\manname{MPI{\tt \char`\_}Pack{\tt \char`\_}external{\tt \char`\_}size}
--- Returns the upper bound on the amount of space needed to pack a message using MPI{\tt \char`\_}Pack{\tt \char`\_}external. 
\subhead{Synopsis}
...
\startarg{size }{output buffer size, in bytes (integer)  
}
\par
...
\location{pack{\tt \char`\_}external{\tt \char`\_}size.c}
\endmanpage
