\startmanpage
\mantitle{MPI{\tt \char`\_}Iprobe}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Iprobe}
...
\startarg{source }{source rank, or  {\tt MPI{\tt \char`\_}ANY{\tt \char`\_}SOURCE} (integer) 
}
\startarg{tag }{tag value or  {\tt MPI{\tt \char`\_}ANY{\tt \char`\_}TAG} (integer) 
}
\startarg{comm }{communicator (handle) 
}
\par
\subhead{Output Parameters}
\startarg{flag }{True if a message with the specified source, tag, and communicator 
