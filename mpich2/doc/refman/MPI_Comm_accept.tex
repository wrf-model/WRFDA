\startmanpage
\mantitle{MPI{\tt \char`\_}Comm{\tt \char`\_}accept}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Comm{\tt \char`\_}accept}
...
\startarg{port{\tt \char`\_}name }{port name (string, used only on root) 
}
...
\startarg{IN }{comm intracommunicator over which call is collective (handle) 
}
\par
\subhead{Output Parameter}
\startarg{newcomm }{intercommunicator with client as remote group (handle) 
}
\par
...
\location{comm{\tt \char`\_}accept.c}
\endmanpage
