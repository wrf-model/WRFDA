\startmanpage
\mantitle{MPI{\tt \char`\_}Type{\tt \char`\_}set{\tt \char`\_}name}{tex}{12/9/2006}
\manname{MPI{\tt \char`\_}Type{\tt \char`\_}set{\tt \char`\_}name}
--- set datatype name 
\subhead{Synopsis}
...
\startarg{type }{datatype whose identifier is to be set (handle) 
}
\startarg{type{\tt \char`\_}name }{the character string which is remembered as the name (string) 
}
\par
...
\location{type{\tt \char`\_}set{\tt \char`\_}name.c}
\endmanpage
