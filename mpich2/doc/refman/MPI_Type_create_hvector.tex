\startmanpage
\mantitle{MPI{\tt \char`\_}Type{\tt \char`\_}create{\tt \char`\_}hvector}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Type{\tt \char`\_}create{\tt \char`\_}hvector}
...
\startarg{count }{number of blocks (nonnegative integer) 
}
\startarg{blocklength }{number of elements in each block (nonnegative integer) 
}
\startarg{stride }{number of bytes between start of each block (integer) 
}
...
\location{type{\tt \char`\_}create{\tt \char`\_}hvector.c}
\endmanpage
