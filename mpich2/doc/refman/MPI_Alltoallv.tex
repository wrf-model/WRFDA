\startmanpage
\mantitle{MPI{\tt \char`\_}Alltoallv}{tex}{10/24/2006}
\manname{MPI{\tt \char`\_}Alltoallv}
...
\startarg{sendcounts }{integer array equal to the group size 
specifying the number of elements to send to each processor 
}
\startarg{sdispls }{integer array (of length group size). Entry 
{\tt j}  specifies the displacement (relative to sendbuf  from
which to take the outgoing data destined for process  {\tt j}  
}
\startarg{sendtype }{data type of send buffer elements (handle) 
}
\startarg{recvcounts }{integer array equal to the group size 
specifying the maximum number of elements that can be received from
each processor 
}
\startarg{rdispls }{integer array (of length group size). Entry 
