\startmanpage
\mantitle{MPI{\tt \char`\_}Reduce}{tex}{11/19/2006}
\manname{MPI{\tt \char`\_}Reduce}
...
\startarg{sendbuf }{address of send buffer (choice) 
}
\startarg{count }{number of elements in send buffer (integer) 
}
\startarg{datatype }{data type of elements of send buffer (handle) 
}
\startarg{op }{reduce operation (handle) 
}
\startarg{root }{rank of root process (integer) 
}
