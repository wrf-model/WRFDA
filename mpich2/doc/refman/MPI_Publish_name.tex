\startmanpage
\mantitle{MPI{\tt \char`\_}Publish{\tt \char`\_}name}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Publish{\tt \char`\_}name}
--- Publish a service name for use with MPI{\tt \char`\_}Comm{\tt \char`\_}connect 
\subhead{Synopsis}
...
\startarg{service{\tt \char`\_}name }{a service name to associate with the port (string) 
}
...
\par
\subhead{Notes}
The maximum size string that may be supplied for {\tt port{\tt \char`\_}name} is
...
\location{publish{\tt \char`\_}name.c}
\endmanpage
