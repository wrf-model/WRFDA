\startmanpage
\mantitle{MPI{\tt \char`\_}Type{\tt \char`\_}create{\tt \char`\_}hindexed}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Type{\tt \char`\_}create{\tt \char`\_}hindexed}
...
\startarg{count }{number of blocks --- also number of entries in 
displacements and blocklengths (integer) 
}
\startarg{blocklengths }{number of elements in each block (array of nonnegative integers) 
}
\startarg{displacements }{byte displacement of each block (array of integer) 
}
...
\location{type{\tt \char`\_}create{\tt \char`\_}hindexed.c}
\endmanpage
