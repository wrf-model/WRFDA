\startmanpage
\mantitle{MPI{\tt \char`\_}Group{\tt \char`\_}excl}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Group{\tt \char`\_}excl}
...
\startarg{ranks }{array of integer ranks in {\tt group} not to appear in {\tt newgroup} 
}
\par
\subhead{Output Parameter}
\startarg{newgroup }{new group derived from above, preserving the order defined by 
{\tt group} (handle) 
}
\par
\subhead{Note}
...
\nextline
\location{group{\tt \char`\_}excl.c}
\endmanpage
