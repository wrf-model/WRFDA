\startmanpage
\mantitle{MPI{\tt \char`\_}Cart{\tt \char`\_}sub}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Cart{\tt \char`\_}sub}
...
\startarg{remain{\tt \char`\_}dims }{the  {\tt i}th entry of remain{\tt \char`\_}dims specifies whether the {\tt i}th 
dimension is kept in the subgrid (true) or is dropped (false) (logical 
vector) 
}
\par
\subhead{Output Parameter}
\startarg{newcomm }{communicator containing the subgrid that includes the calling 
process (handle) 
}
\par
