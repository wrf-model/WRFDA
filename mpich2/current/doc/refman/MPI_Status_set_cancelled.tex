\startmanpage
\mantitle{MPI{\tt \char`\_}Status{\tt \char`\_}set{\tt \char`\_}cancelled}{tex}{12/9/2006}
\manname{MPI{\tt \char`\_}Status{\tt \char`\_}set{\tt \char`\_}cancelled}
...
\startarg{status }{status to associate cancel flag with (Status)
}
\startarg{flag }{if true indicates request was cancelled (logical)
}
\par
...
\location{status{\tt \char`\_}set{\tt \char`\_}cancelled.c}
\endmanpage
