\startmanpage
\mantitle{MPI{\tt \char`\_}Graph{\tt \char`\_}create}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Graph{\tt \char`\_}create}
...
\startarg{comm{\tt \char`\_}old }{input communicator without topology (handle) 
}
\startarg{nnodes }{number of nodes in graph (integer) 
}
\startarg{index }{array of integers describing node degrees (see below) 
}
\startarg{edges }{array of integers describing graph edges (see below) 
}
\startarg{reorder }{ranking may be reordered (true) or not (false) (logical) 
}
\par
\subhead{Output Parameter}
\startarg{comm{\tt \char`\_}graph }{communicator with graph topology added (handle) 
}
\par
\subhead{Notes}
...
\par
\subhead{Algorithm}
We ignore the {\tt reorder} info currently.
\par
