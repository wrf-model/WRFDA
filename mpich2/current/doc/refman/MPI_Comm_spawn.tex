\startmanpage
\mantitle{MPI{\tt \char`\_}Comm{\tt \char`\_}spawn}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Comm{\tt \char`\_}spawn}
...
\startarg{command }{name of program to be spawned (string, significant only at root) 
}
\startarg{argv }{arguments to command (array of strings, significant only at root) 
}
\startarg{maxprocs }{maximum number of processes to start (integer, significant only 
at root) 
}
\startarg{info }{a set of key-value pairs telling the runtime system where and how 
...
\startarg{intercomm }{intercommunicator between original group and the 
newly spawned group (handle) 
}
\startarg{array{\tt \char`\_}of{\tt \char`\_}errcodes }{one code per process (array of integer) 
}
\par
...
\location{comm{\tt \char`\_}spawn.c}
\endmanpage
