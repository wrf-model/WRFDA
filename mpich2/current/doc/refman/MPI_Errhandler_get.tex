\startmanpage
\mantitle{MPI{\tt \char`\_}Errhandler{\tt \char`\_}get}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Errhandler{\tt \char`\_}get}
...
\startarg{comm }{communicator to get the error handler from (handle) 
}
\par
\subhead{Output Parameter}
\startarg{errhandler }{MPI error handler currently associated with communicator
(handle) 
}
\par
...
\par
\subhead{Note on Implementation}
\par
...
\location{errhandler{\tt \char`\_}get.c}
\endmanpage
