\startmanpage
\mantitle{MPI{\tt \char`\_}Win{\tt \char`\_}set{\tt \char`\_}name}{tex}{12/9/2006}
\manname{MPI{\tt \char`\_}Win{\tt \char`\_}set{\tt \char`\_}name}
...
\startarg{win }{window whose identifier is to be set (handle) 
}
\startarg{win{\tt \char`\_}name }{the character string which is remembered as the name (string) 
}
\par
...
\location{win{\tt \char`\_}set{\tt \char`\_}name.c}
\endmanpage
