\startmanpage
\mantitle{MPI{\tt \char`\_}Graph{\tt \char`\_}map}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Graph{\tt \char`\_}map}
...
\startarg{nnodes }{number of graph nodes (integer) 
}
\startarg{index }{integer array specifying the graph structure, see {\tt MPI{\tt \char`\_}GRAPH{\tt \char`\_}CREATE} 
}
\startarg{edges }{integer array specifying the graph structure 
}
\par
\subhead{Output Parameter}
\startarg{newrank }{reordered rank of the calling process; {\tt MPI{\tt \char`\_}UNDEFINED} if the 
