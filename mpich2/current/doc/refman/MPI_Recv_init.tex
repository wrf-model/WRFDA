\startmanpage
\mantitle{MPI{\tt \char`\_}Recv{\tt \char`\_}init}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Recv{\tt \char`\_}init}
...
\startarg{count }{number of elements received (integer) 
}
\startarg{datatype }{type of each element (handle) 
}
\startarg{source }{rank of source or {\tt MPI{\tt \char`\_}ANY{\tt \char`\_}SOURCE} (integer) 
}
\startarg{tag }{message tag or {\tt MPI{\tt \char`\_}ANY{\tt \char`\_}TAG} (integer) 
}
...
\par
\subhead{See Also}
 MPI{\tt \char`\_}Start, MPI{\tt \char`\_}Startall, MPI{\tt \char`\_}Request{\tt \char`\_}free
\nextline
\location{recv{\tt \char`\_}init.c}
\endmanpage
