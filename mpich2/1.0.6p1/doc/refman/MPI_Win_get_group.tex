\startmanpage
\mantitle{MPI{\tt \char`\_}Win{\tt \char`\_}get{\tt \char`\_}group}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Win{\tt \char`\_}get{\tt \char`\_}group}
...
\startarg{group }{group of processes which share access to the window (handle) 
}
\par
\subhead{Notes}
The group is a duplicate of the group from the communicator used to
create the MPI window, and should be freed with {\tt MPI{\tt \char`\_}Group{\tt \char`\_}free} when
it is no longer needed.  This group can be used to form the group of
neighbors for the routines {\tt MPI{\tt \char`\_}Win{\tt \char`\_}post} and {\tt MPI{\tt \char`\_}Win{\tt \char`\_}start}.
\par
...
\location{win{\tt \char`\_}get{\tt \char`\_}group.c}
\endmanpage
