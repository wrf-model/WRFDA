\startmanpage
\mantitle{MPI{\tt \char`\_}Gather}{tex}{10/24/2006}
\manname{MPI{\tt \char`\_}Gather}
...
\startarg{recvcount }{number of elements for any single receive (integer, 
significant only at root) 
}
\startarg{recvtype }{data type of recv buffer elements 
(significant only at root) (handle) 
}
\startarg{root }{rank of receiving process (integer) 
}
\startarg{comm }{communicator (handle) 
}
\par
\subhead{Output Parameter}
\startarg{recvbuf }{address of receive buffer (choice, significant only at {\tt root}) 
}
\par
