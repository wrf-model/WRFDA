\startmanpage
\mantitle{MPI{\tt \char`\_}Unpack{\tt \char`\_}external}{tex}{10/17/2006}
\manname{MPI{\tt \char`\_}Unpack{\tt \char`\_}external}
...
\startarg{insize }{input buffer size, in bytes (integer)  
}
\startarg{outcount }{number of output data items (integer)  
}
\startarg{datatype }{datatype of output data item (handle)  
}
\par
\subhead{Input/Output Parameter}
\startarg{position }{current position in buffer, in bytes (integer)  
}
\par
...
\location{unpack{\tt \char`\_}external.c}
\endmanpage
