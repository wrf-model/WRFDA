\startmanpage
\mantitle{MPI{\tt \char`\_}Group{\tt \char`\_}incl}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Group{\tt \char`\_}incl}
...
\startarg{n }{number of elements in array {\tt ranks} (and size of newgroup ) (integer) 
}
\startarg{ranks }{ranks of processes in {\tt group} to appear in {\tt newgroup} (array of 
integers) 
}
\par
\subhead{Output Parameter}
\startarg{newgroup }{new group derived from above, in the order defined by {\tt ranks} 
(handle) 
}
\par
...
\nextline
\location{group{\tt \char`\_}incl.c}
\endmanpage
