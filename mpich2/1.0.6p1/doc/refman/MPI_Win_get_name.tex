\startmanpage
\mantitle{MPI{\tt \char`\_}Win{\tt \char`\_}get{\tt \char`\_}name}{tex}{12/9/2006}
\manname{MPI{\tt \char`\_}Win{\tt \char`\_}get{\tt \char`\_}name}
...
\startarg{win }{window whose name is to be returned (handle) 
}
\par
\subhead{Output Parameters}
\startarg{win{\tt \char`\_}name }{the name previously stored on the window, or a empty string if 
...
\location{win{\tt \char`\_}get{\tt \char`\_}name.c}
\endmanpage
