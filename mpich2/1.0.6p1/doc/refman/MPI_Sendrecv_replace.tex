\startmanpage
\mantitle{MPI{\tt \char`\_}Sendrecv{\tt \char`\_}replace}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Sendrecv{\tt \char`\_}replace}
...
\startarg{count }{number of elements in send and receive buffer (integer) 
}
\startarg{datatype }{type of elements in send and receive buffer (handle) 
}
\startarg{dest }{rank of destination (integer) 
}
\startarg{sendtag }{send message tag (integer) 
}
\startarg{source }{rank of source (integer) 
}
\startarg{recvtag }{receive message tag (integer) 
}
\startarg{comm }{communicator (handle) 
}
\par
\subhead{Output Parameters}
\startarg{buf }{initial address of send and receive buffer (choice) 
}
...
\startarg{MPI{\tt \char`\_}ERR{\tt \char`\_}TRUNCATE }{Message truncated on receive.  The buffer size specified
...
\par
\location{sendrecv{\tt \char`\_}rep.c}
\endmanpage
