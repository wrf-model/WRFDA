\startmanpage
\mantitle{MPI{\tt \char`\_}Error{\tt \char`\_}string}{tex}{12/9/2006}
\manname{MPI{\tt \char`\_}Error{\tt \char`\_}string}
...
\startarg{errorcode }{Error code returned by an MPI routine or an MPI error class
}
\par
\subhead{Output Parameter}
\startarg{string }{Text that corresponds to the errorcode 
}
\startarg{resultlen }{Length of string 
}
\par
Notes:  Error codes are the values return by MPI routines (in C) or in the
{\tt ierr} argument (in Fortran).  These can be converted into error classes
with the routine {\tt MPI{\tt \char`\_}Error{\tt \char`\_}class}.
\par
...
\location{error{\tt \char`\_}string.c}
\endmanpage
