\startmanpage
\mantitle{MPI{\tt \char`\_}Group{\tt \char`\_}range{\tt \char`\_}excl}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Group{\tt \char`\_}range{\tt \char`\_}excl}
...
\startarg{n }{number of elements in array {\tt ranks} (integer) 
}
...
{\tt group}  of processes to be excluded from the output group {\tt newgroup} .  
}
\par
\subhead{Output Parameter}
\startarg{newgroup }{new group derived from above, preserving the 
order in {\tt group}  (handle) 
}
\par
\subhead{Note}
...
\nextline
\location{group{\tt \char`\_}range{\tt \char`\_}excl.c}
\endmanpage
