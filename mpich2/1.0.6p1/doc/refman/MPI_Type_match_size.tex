\startmanpage
\mantitle{MPI{\tt \char`\_}Type{\tt \char`\_}match{\tt \char`\_}size}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Type{\tt \char`\_}match{\tt \char`\_}size}
...
\startarg{typeclass }{generic type specifier (integer) 
}
\startarg{size }{size, in bytes, of representation (integer) 
}
\par
\subhead{Output Parameter}
\startarg{type }{datatype with correct type, size (handle) 
}
\par
\subhead{Notes}
...
\location{type{\tt \char`\_}match{\tt \char`\_}size.c}
\endmanpage
