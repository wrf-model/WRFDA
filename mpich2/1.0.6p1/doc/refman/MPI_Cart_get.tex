\startmanpage
\mantitle{MPI{\tt \char`\_}Cart{\tt \char`\_}get}{tex}{5/8/2006}
\manname{MPI{\tt \char`\_}Cart{\tt \char`\_}get}
...
\startarg{maxdims }{length of vectors  {\tt dims}, {\tt periods}, and {\tt coords
}in the calling program (integer) 
}
\par
\subhead{Output Parameters}
\startarg{dims }{number of processes for each cartesian dimension (array of integer) 
}
\startarg{periods }{periodicity (true/false) for each cartesian dimension 
(array of logical) 
}
\startarg{coords }{coordinates of calling process in cartesian structure 
(array of integer) 
}
\par
