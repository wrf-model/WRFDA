\startmanpage
\mantitle{MPI{\tt \char`\_}Pack{\tt \char`\_}external}{tex}{10/17/2006}
\manname{MPI{\tt \char`\_}Pack{\tt \char`\_}external}
...
\startarg{incount }{number of input data items (integer)  
}
\startarg{datatype }{datatype of each input data item (handle)  
}
\startarg{outsize }{output buffer size, in bytes (integer)  
}
\par
...
\location{pack{\tt \char`\_}external.c}
\endmanpage
