!  program to update BC file from wrfvar output.
...
!--Get mu, mub, msfu, and msfv
   ...
   do n=1,num2d
      call da_get_dims_cdf( wrf_3dvar_output_file, trim(var2d(n)), dims, ndims, debug )
      ...
      select case(trim(var2d(n)))
         case ('MU') ;
            ...
            call da_get_var_2d_real_cdf( wrf_3dvar_output_file, trim(var2d(n)), mu, &
...
!           var_pref = 'R' // trim(var3d(n))
            ...
            call da_get_var_3d_real_cdf( wrf_3dvar_output_file, trim(var3d(n)), full3d, &
                                      dims(1), dims(2), dims(3), 1, debug )
