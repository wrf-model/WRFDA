...
while (<DATA>) {
     ...
     next if /^#/;
     if ( /^(\D)/ ) {
         ($Arch, $Source, $Compiler, $Cpunumber, $Project, $Queue, $Database, $Baseline) = 
               split /\s+/,$_;
     }
...
printf "#INDEX   EXPERIMENT                   CPU    PAROPT\n";
printf "%-4d     %-27s  %-4d   "."%-10s "x(keys %{$Experiments{$_}{paropt}})."\n", 
     $Experiments{$_}{index}, $_, $Experiments{$_}{cpunumber},  keys%{$Experiments{$_}{paropt}}
     ...
     foreach my $name (keys %Experiments) {
         ...
         foreach my $par (sort keys %{$Experiments{$name}{paropt}}) {
             ...
             unless ( defined $Experiments{$name}{paropt}{$par}{jobid} ) {      #  to be submitted .
                 ...
                 my $rc = &submit_job ( $name, $Compiler, $par, $Experiments{$name}{cpunumber} );
                 if (defined $rc) { 
...
sub refresh_status {
    ...
    push @mes, "Experiment                  Paropt      CPU    Status    Walltime(s)    Compare\n";

    foreach my $name (sort keys %Experiments) {
        foreach my $par (sort keys %{$Experiments{$name}{paropt}}) {
            push @mes, sprintf "%-28s%-12s%-7d%-10s%-15d%-7s\n", 
                    $name, $par, $Experiments{$name}{cpunumber}, 
...
#INDEX   EXPERIMENT                  CPU            PAROPT
1        tutorial_xinzhang           16             serial|smpar|dmpar
2        cv3_guo                     32             serial|smpar|dmpar
3        t44_liuz                    32             serial|smpar|dmpar
4        radar_meixu                 32             serial|smpar|dmpar
5        cwb_ascii                   32             serial|smpar|dmpar
