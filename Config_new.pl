...
#   look for each known option
 ...
 if ( $sw_dmparallel eq "RSL_LITE" ) 
 {
  ...
  $sw_comms_lib = "\$(WRFDA_SRC_ROOT_DIR)/external/RSL_LITE/librsl_lite.a" ;
  $sw_comms_external = "\$(WRFDA_SRC_ROOT_DIR)/external/RSL_LITE/librsl_lite.a gen_comms_rsllite module_dm_rsllite" ;
  $sw_comms_include = "-I\$(WRFDA_SRC_ROOT_DIR)/external/RSL_LITE" ;
 }
...
while ( <CONFIGURE_DEFAULTS> )
{
  ...
  if ( $latchon == 1 )
  {
	...
	$_ =~ s:CONFIGURE_NETCDF_FLAG:-DNETCDF: ;
        if ( $sw_os == Interix ) {
	  $_ =~ s:CONFIGURE_NETCDF_LIB_PATH:\$\(WRFDA_SRC_ROOT_DIR\)/external/io_netcdf/libwrfio_nf.a -L$sw_netcdf_path/lib $sw_usenetcdff -lnetcdf : ;
        } else {
	  $_ =~ s:CONFIGURE_NETCDF_LIB_PATH:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/io_netcdf -lwrfio_nf -L$sw_netcdf_path/lib $sw_usenetcdff -lnetcdf : ;
        }
	 }
    else                   
	...
	$_ =~ s:CONFIGURE_PNETCDF_FLAG:-DPNETCDF: ;
        if ( $sw_os == Interix ) {
	  $_ =~ s:CONFIGURE_PNETCDF_LIB_PATH:\$\(WRFDA_SRC_ROOT_DIR\)/external/io_pnetcdf/libwrfio_pnf.a -L$sw_pnetcdf_path/lib -lpnetcdf: ;
        } else {
	  $_ =~ s:CONFIGURE_PNETCDF_LIB_PATH:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/io_pnetcdf -lwrfio_pnf -L$sw_pnetcdf_path/lib -lpnetcdf: ;
        }
	 }
    else                   
	...
	$_ =~ s:CONFIGURE_PHDF5_LIB_PATH:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/io_phdf5 -lwrfio_phdf5 -L$sw_phdf5_path/lib -lhdf5_fortran -lhdf5 -lm -lz -L$sw_phdf5_path/lib -lsz: ;
	 }
    else                   
    ...
    if ( $sw_jasperlib_path && $sw_jasperinc_path ) 
      { $_ =~ s/CONFIGURE_WRFIO_GRIB2/wrfio_grib2/g ;
        ...
        $_ =~ s:CONFIGURE_GRIB2_LIB:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/io_grib2 -lio_grib2 -L$sw_jasperlib_path -ljasper:g ;
      }
    else                   
    ...
    if ( $sw_esmflib_path && $sw_esmfinc_path )
      ...
      $_ =~ s:ESMFIOLIB:-L$sw_esmflib_path -lesmf -L\$\(WRFDA_SRC_ROOT_DIR\)/external/io_esmf -lwrfio_esmf \$\(ESMF_LIB_FLAGS\):g ;
      $_ =~ s:ESMFIOEXTLIB:-L$sw_esmflib_path -lesmf -L\$\(WRFDA_SRC_ROOT_DIR\)/external/io_esmf -lwrfio_esmf \$\(ESMF_LIB_FLAGS\):g ;
    ...
    else
      {
        ...
        $_ =~ s:ESMFLIBFLAG::g ;
        if ( $sw_os == Interix ) {
           $_ =~ s:ESMFIOLIB:\$\(WRFDA_SRC_ROOT_DIR\)/external/esmf_time_f90/libesmf_time.a:g ;
           $_ =~ s:ESMFIOEXTLIB:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/esmf_time_f90/libesmf_time.a:g ;
        } else {
           $_ =~ s:ESMFIOLIB:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/esmf_time_f90 -lesmf_time:g ;
           $_ =~ s:ESMFIOEXTLIB:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/esmf_time_f90 -lesmf_time:g ;
        }
      }
     if ( $ENV{HWRF} )
       {
        $_ =~ s:CONFIGURE_ATMPOM_LIB:-L\$\(WRFDA_SRC_ROOT_DIR\)/external/atm_pom  -latm_pom:g ;
        $_ =~ s:CONFIGURE_ATMPOM_INC:-I\$\(WRFDA_SRC_ROOT_DIR\)/external/atm_pom:g;
...
while ( <ARCH_PREAMBLE> )
  ...
  if ( $sw_esmflib_path && $sw_esmfinc_path )
    ...
    $_ =~ s:ESMFMODDEPENDENCE:\$\(WRFDA_SRC_ROOT_DIR\)/external/io_esmf/module_utility.o:g ;
    $_ =~ s:ESMFMODINC:-I$sw_esmfinc_path -I\$\(WRFDA_SRC_ROOT_DIR\)/main:g ;
    $_ =~ s:ESMFIOINC:-I\$\(WRFDA_SRC_ROOT_DIR\)/external/io_esmf:g ;
    ...
    $_ =~ s:ESMFMODDEPENDENCE:\$\(WRFDA_SRC_ROOT_DIR\)/external/esmf_time_f90/module_utility.o:g ;
    $_ =~ s:ESMFMODINC::g ;
    $_ =~ s:ESMFIOINC:-I\$\(WRFDA_SRC_ROOT_DIR\)/external/esmf_time_f90:g ;
    $_ =~ s:ESMFIODEFS::g ;
