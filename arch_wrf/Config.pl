...
until ( $validresponse ) {
  ...
  $opt = 1 ;
  open CONFIGURE_DEFAULTS, "< ./arch_wrf/configure.defaults" 
      or die "Cannot open ./arch_wrf/configure.defaults for reading" ;
...
#printf "     arch_wrf/configure.defaults\n" ;
#printf "\n" ;

open CONFIGURE_WRF, "> configure.wrf" or die "cannot append configure.wrf" ;
open ARCH_PREAMBLE, "< arch_wrf/preamble" or die "cannot open arch_wrf/preamble" ;
my @preamble;
...
open ARCH_POSTAMBLE, "< arch_wrf/postamble" or die "cannot open arch_wrf/postamble" ;
