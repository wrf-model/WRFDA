...
export START_DATE=$($WRFVAR_DIR/build/da_advance_time.exe ${DATE} $WINDOW_START 2>/dev/null)
export END_DATE=$($WRFVAR_DIR/build/da_advance_time.exe ${DATE} $WINDOW_END 2>/dev/null)
...
while [[ $LOCAL_DATE -le $END_DATE ]]; do
   ...
   LOCAL_DATE=$($WRFVAR_DIR/build/da_advance_time.exe ${LOCAL_DATE} ${OBS_FREQ} 2>/dev/null)
done
