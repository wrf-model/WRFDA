...
#@ queue
#-----------------------------------------------------------------------
# Script DA_Run_WRF-4DVar_serial.csh
#
# Purpose: Top level script for running the WRF-4DVar system on single CPU
#
...
setenv DA_NTMAX                 100              # Maximum number of inner loop iterations.
...
setenv USER_DIR /ptmp/xinzhang/wrf.QN_DFI    # user data and results
...
setenv BE_NAME gen_be_nmc_${CASE_NAME}${RESOLUTION}_${WEST_EAST_GRID_NUMBER}x${SOUTH_NORTH_GRID_NUMBER}x${VERTICAL_GRID_NUMBER}.dat    						# The BE file name
...
#Supported default WRF-Var namelist variables:
 ...
 if ( ! $?DA_CV_OPTIONS ) setenv DA_CV_OPTIONS 5                  # Background error statistics: 2=NCAR, 3=NCEP.
 ...
 if ( ! $?DA_NTMAX )         set DA_NTMAX =   5                   # Maximum number of inner loop iterations.
...
echo '#-----------------------------------------------------------------------'
 ...
 ln -sf $EXE_DIR/wrfplus_serial.exe wrf.exe
 ln -sf $EXE_DIR/4dvar_serial.exe da_3dvar.exe
...
mv run*out* stdout.* wrf_3dvar_output ${RES_DIR}/
