...
set echo
 ...
 setenv WRF_DIR /snowdrift/users/dmbarker/code_development/wrf3dvar 
 setenv DAT_DIR /data3/dmbarker/data/amps2/noobs/gen_be_input

 setenv RUN_DIR /snowdrift/users/dmbarker/gen_be
 mkdir -p $RUN_DIR

 set BE_TYPE = 1          # 1: NMC(Default),  2: ENS 
 set MEMBER  = 1          # (note) if BE_TYPE = 1, MEMBER should be 1

 setenv WEST_EAST_GRID_NUMBER   220
 setenv SOUTH_NORTH_GRID_NUMBER 208
 setenv VERTICAL_GRID_NUMBER    32
 setenv GRID_DISTANCE           30000

#set date
 set START_BE_DATE = 2004-05-30_00:00:00
 set END_BE_DATE   = 2004-05-30_12:00:00
 ...
 set TIME0    = ${START_CY}-${START_MM}-${START_DD}_${START_HH}
 ...
 set TIME1  = ${END_CY}${END_MM}${END_DD}${END_HH}

 set file_name     = wrfout_d01    # users' data file root name
 set file_root     = wrf_forecast  # file name used in program

 set file_interval = 12

# ------------------------------------------------------------------
# To link the users' data files to the program assigned file names:

 set HH = $START_HH
 set DD = $START_DD
 set MM = $START_MM
 set YY = $START_CY
 set TIMEn = ${YY}${MM}${DD}${HH}

# ..................................................................  
  @ Leap_year = $YY % 4
  if ( $Leap_year == 0 ) then
    set Max_year_days = 366
  else
    set Max_year_days = 365
  endif
  set MMDAYS = $NDAYS[$MM]
  if ( ( $MM == 02 ) && ( $Leap_year == 0 ) ) @ MMDAYS = $MMDAYS + 1
# ..................................................................
 ...
 while ( $TIMEn < $TIME1 )
      ...
      if ( $HH < 10 && $HH != 00 ) set HH = 0${HH}

      set  TIME = ${YY}-${MM}-${DD}_${HH}
      ln -sf ${file_name}_${TIME}:00:00 ${file_root}_${TIME}:00:00
      set TIMEn = ${YY}${MM}${DD}${HH}
...
cat >! namelist.stats << EOF
 &FILERECD
  ...
  PRINT_DETAIL    = 0, 
  from_mss        = .FALSE.,
  mss_directory   = 'MSLEE/MODL/DAOU/C-100km'/
...
ln -sf $DAT_DIR/${file_root}_${TIME0}:00:00  wrf_3dvar_input
