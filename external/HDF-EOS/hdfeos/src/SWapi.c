...
SWindexinfo(int32 regionID, char *object, int32 *rank, char *dimlist, int32 *indices[])
{
  ...
  for (j = 0; j < *rank; j++)
  {
      if (SWXRegion[regionID]->StartVertical[j] == -1)
      {
          indices[j][0] = SWXRegion[regionID]->StartRegion[j];
      }
      else
      {
          indices[j][0] = SWXRegion[regionID]->StartVertical[j];
      }


      if (SWXRegion[regionID]->StopVertical[j] == -1)
      {
          if (SWXRegion[regionID]->StopRegion[j] == 0)
          {
              indices[j][1] = dims[j];
          }
          else
          {
              indices[j][1] = SWXRegion[regionID]->StopRegion[j];
          }
      }
      else
      {
          if (SWXRegion[regionID]->StopVertical[j] == 0)
          {
              indices[j][1] = dims[j];
          }
          else
          {
              indices[j][1] = SWXRegion[regionID]->StopVertical[j];
          }
      }
...
|  FUNCTION: SWidxinfo                                                        |
|                                                                             |
|  DESCRIPTION: FORTRAN wrapper around SWidxinfo                              |
|                                                                             |
|                                                                             |
|  Return Value    Type     Units     Description                             |
|  ============   ======  =========   =====================================   |
