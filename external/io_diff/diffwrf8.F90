...
contains
   ...
   subroutine help
     ...
     write(*,'(8x, "wrf fl : WRF file name to read.")')
...
!    write(*,*)'Next Var |',TRIM(VarName),'|'
      ...
      if ( VarName .eq. name ) then
        ...
        call ext_ncd_read_field(dh1,DateStr,TRIM(name),data,WRF_REAL8,0,0,0,ord, &
                            staggering, dimnames ,                      &
...
!      ENDIF
      ...
      if( WrfType == WRF_REAL8) then
        ...
        call ext_ncd_read_field(dh1,DateStr,TRIM(VarName),data,WRF_REAL8,0,0,0,ord,&
                            staggering, dimnames ,                      &
                            start_index,end_index,                      & !dom 
                            start_index,end_index,                      & !mem
                            start_index,end_index,                      & !pat
	  ...
	  write(*,*)'Error reading ',Varname,' from ',flnm
          ...
          write(*,*)'  end_index(3) ',end_index(3)
        ENDIF
        call ext_ncd_read_field(dh2,DateStr,TRIM(VarName),data2,WRF_REAL8,0,0,0,ord,&
                            staggering, dimnames ,                      &
                            start_index,end_index,                      & !dom 
                            start_index,end_index,                      & !mem
                            start_index,end_index,                      & !pat
