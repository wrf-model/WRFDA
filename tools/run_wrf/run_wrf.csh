...
if ( ! $?CYCLE_PERIOD )     setenv CYCLE_PERIOD 6            # Assimilation frequency.
...
if ( ! $?REGION )           setenv REGION con100             # Region name. 
...
if ( ! $?EXPT )             setenv EXPT test                 # Experiment name.
...
if ( ! $?XDIM )             setenv XDIM 90                   # 
if ( ! $?YDIM )             setenv YDIM 90                   # 
...
if ( ! $?DELTA_X )          setenv DELTA_X 100000            # Resolution (m). 
if ( ! $?DELTA_Y )          setenv DELTA_Y 100000            # Resolution (m).
...
if ( ! $?DA_CU_PHYSICS )    setenv DA_CU_PHYSICS 1           #(1=, 2=,3=).
if ( ! $?DA_MP_ZERO_OUT )   setenv DA_MP_ZERO_OUT 0          # 
...
if ( ! $?DA_TIME_STEP_SOUND ) setenv DA_TIME_STEP_SOUND 4    # 
...
#  Link MPI node information:
   ...
   if ( $RUN_REAL == .TRUE. ) then
      echo "---------------------------------------------------------------"
      echo "3) Run WRF real.exe on ${NUM_PROCS} processors."
      echo "---------------------------------------------------------------"
      echo ""
...
#     Run real:
      mpirun -v -nolocal -np ${NUM_PROCS} -machinefile nodes ./real.exe >&! /dev/null
   ...
   if ( $RUN_VAR == .TRUE. ) then
      echo "---------------------------------------------------------------"
      echo "5) Run WRF-Var on ${NUM_PROCS} processors."
      echo "---------------------------------------------------------------"
      echo ""
