...
gen_i1_decls ( char * dn )
{
  ...
  {
    strcpy(corename,get_corename_i(i)) ;
    if ( strlen(dn) > 0 ) { sprintf(fname,"%s/%s%s",dn,corename,fn2) ; }
    else                  { sprintf(fname,"%s%s",corename,fn2) ; }
    if ((fp = fopen( fname , "w" )) == NULL ) continue ;
    print_warning(fp,fname) ;
    gen_decls1 ( fp , corename, &Domain , GRIDREF , NOPOINTERDECL , I1 , MEDIATION_LAYER ) ;
...
int
gen_decls1( FILE * fp , char * corename , node_t * node , int sw_ranges, int sw_point , int mask , int layer )
{
  node_t * p ; 
  int var, tag, ipass ;
  char fname[NAMELEN], post[NAMELEN] ;
  ...
  for ( p = node->fields ; p != NULL ; p = p->next )
  {
    if ( p->node_kind & mask )
    {
	  ...
	  case ARGADJ :
        ...
        /*          type dim pdecl   name */
        if ( ! strcmp( field_type( t1, p ), "real") ) fprintf(fp, "%s = 0.0 \n",
                    fname ) ;
      }
      } /* End Wei's change */
    }
