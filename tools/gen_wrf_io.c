...
fprintf(fp,"          %s_desc_table( grid%%id, i  ), & ! Desc\n",p->name) ;
fprintf(fp,"          %s_units_table( grid%%id, i  ), & ! Units\n",p->name) ;
        }
fprintf(fp,"'%s ext_write_field '//TRIM(%s_dname_table( grid%%id, i ))//' memorder XZY' , & ! Debug message\n", fname, p->name ) ;
        /* global dimensions */
...
fprintf(fp,"          fid                             , &  ! DataHandle\n") ;
fprintf(fp,"          current_date(1:19)              , &  ! DateStr\n") ; 
fprintf(fp,"          TRIM(%s_dname_table( grid%%id, i )) // '%s', & !data name\n",p->name,bdytag) ;
            if ( ok_to_collect_distribute ) {
fprintf(fp,"                       globbuf_%s               , &  ! Field \n",p->members->type->name ) ;
            } else {
              strcpy(bdytag2,"") ;
              strncat(bdytag2,bdytag, pass+2) ;
fprintf(fp,"          grid%%%s%s(1,kds,1,%d,i)  , &  ! Field\n",p->name,bdytag2,ibdy) ;
            }
fprintf(fp,"          WRF_%s             , &  ! FieldType \n" , p->members->type->name ) ;
fprintf(fp,"          grid%%communicator  , &  ! Comm\n") ;
fprintf(fp,"          grid%%iocommunicator  , &  ! Comm\n") ;
