...
/* ////////  BOUNDARY ///////////////////// */
	  ...
	  if ( sw_io == GEN_INPUT )
	  {
          for ( var_tmp = 0 ; var_tmp < p->var_numb ; var_tmp++ )
          { 
            if(var_tmp)
            {
              if ( !strncmp( p->use, "dyn_", 4 ) )
              {
                if ( p->var_name[var_tmp] == 'a' )
                  fprintf(fp,"IF ( grid%%dyn_opt .EQ. %s_AD ) THEN\n",p->use) ;
                else if ( p->var_name[var_tmp] == 'g' )
	      ...
	      fprintf(fp,"IF ( wrf_dm_on_monitor() ) THEN\n") ;
            ...
            if(var_tmp)
            { 
              fprintf(fp,"                       '%c_%s'             , &  ! Data Name \n", toupper(p->var_name[var_tmp]), dname ) ;
              fprintf(fp,"                       %s%s%c_%s(1,kds,1,%d)     , &  ! Field \n" , structname , core , p->var_name[var_tmp], p->name, ibdy ) ;
            }
            else 
            {
