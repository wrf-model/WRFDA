...
!  the bin information from wrf3dvar/gen_be:
   if (code_version == 'wrf3dvar') &
...
! 3.2 Setup the bin information
    ...
    if (code_version == 'wrfvar  ') then

! 3.2.1 Read the bin information generated by wrfvar/gen_be:
 
   filename = trim(gen_be_dir)//'/bin.data'
   open (input_unit+1, file = filename, form='unformatted')
   read(input_unit+1)bin_type
   read(input_unit+1)lat_min, lat_max, binwidth_lat
   read(input_unit+1)hgt_min, hgt_max, binwidth_hgt
   read(input_unit+1)num_bins, num_bins2d
   read(input_unit+1)bin(1:ni,1:nj,1:nk)
   read(input_unit+1)bin2d(1:ni,1:nj)
   close(input_unit+1)

    else if (code_version == 'wrf3dvar') then

! 3.2.2 the bin information from wrf3dvar/gen_be 
