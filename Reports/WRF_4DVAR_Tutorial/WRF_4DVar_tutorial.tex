...
\title[WRF 4D-Var V3.6, WRFDA Tutorial]{\small{WRF Four-dimensional variational data assimilation system\\Tutorial for V3.6}}
\author[Zhang and Huang, Boulder, July 2014]{Xin Zhang \and Xiang-Yu Huang}

\date{\tiny{Presented in July 2014, WRFDA Tutorial} \\
  ~\\
  ~\\
  ~\\
  ~\\
...
\begin{itemize}
	...
	\item Knowledge and experience to run WRFDA (3D-Var)
...
\begin{itemize}
	...
	\item The dynamics and physics of the forecast model is an integral part of 4D-Var, so observations are used in a meteorologically more consistent way \pause
...
\includegraphics[scale=0.25]{jcdf_formular} & \includegraphics[scale=0.25]{jcdf}
\end{tabular}
}
...
\begin{itemize}
	...
	\item download the WRFPLUSV3 codes  and the patch from :\\
		...
		{\tiny\url{http://www2.mmm.ucar.edu/wrf/users/wrfda/known-prob.html\#V3_6}}\pause
	\item enter into WRFDA/var/test/4dvar \pause
	\item get the test dataset from :\\
		{\tiny\url{ftp://ftp.ucar.edu/pub/mmm/xinzhang/WRF4DVar_V3.6_Tutorial/2008020521}}\pause
...
We have tested the WRF 4D-Var V3.6 on following systems:
\begin{itemize}
...
Taylor formula:
\begin{equation*}
\lim_{\alpha \to 0}\frac{\|M(\mathbf{x}+\alpha\delta\mathbf{x})-M(\mathbf{x})\|}{\|\mathbf{M}(\alpha\delta\mathbf{x})\|}=1
\end{equation*}
...
adjoint identity:
\begin{equation*}
\forall\mathbf{x}, \forall\mathbf{y} : \langle{\mathbf{M}\cdot\mathbf{x},\mathbf{y}}\rangle =  \langle{\mathbf{x},\mathbf{M^{*}}\cdot\mathbf{y}}\rangle
\end{equation*}
\begin{block}{check results}
{\tiny{
\begin{verbatim}
...
\tiny{\url{http://www.mmm.ucar.edu/wrf/users/wrfda/Docs/user_guide_V3.6/users_guide_chap6.htm#_Radiance_Data_Assimilations}} \\
~\\
