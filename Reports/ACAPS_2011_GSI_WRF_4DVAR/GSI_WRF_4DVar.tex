...
\begin{itemize}
	\item This presentation reports the status of the development of GSI-based WRF 4DVAR as implemented in Boulder's version of May 2011\pause
	...
	\item The WRF tangent linear and adjoint codes V3.3 (hereafter, WRFPLUSV3)  has been re-written from scratch to be consistent with the latest WRF repository codes \pause
	\item Because the parallelization of the latest WRFPLUSV3 is still on going, only 1 processor parallel run is doable at this moment
	\end{itemize}
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{New WRFPLUSV3}

\frame{
\frametitle{Major Improvements of WRFPLUSV3}
\begin{itemize}
   	\item Re-written WRF adjoint and tangent linear codes based on the latest WRF repository codes. \pause
	...
	\item Adding option to control if all inputs and outputs were happen in disk or memory, so WRFPLUSV3 can be used as a standalone tool or as a component in 4DVAR system.
\end{itemize}
}
...
\begin{itemize}
	...
	\item Added a new module which serves the hub between GSI and WRFPLUSV3 \pause
