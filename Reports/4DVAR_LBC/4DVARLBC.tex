...
\begin{equation}
J_\mathrm{lbc}=\frac{1}{2}(\delta{\mathbf{x}_\mathrm{lbc}}(t_k))^T\bm{\mathbf{B}}_\mathrm{lbc}^{-1}\delta{\mathbf{x}_\mathrm{lbc}}(t_k)
\end{equation}
where $\bm{\mathbf{B}}_\mathrm{lbc}$ represents the covariance of the lateral boundary condition 
...
conditions, we will simply apply $\bm{\mathbf{B}}_\mathrm{lbc}=\bm{\mathbf{B}}$. 
...
%the lateral boundary relaxation schemes, where 
The WRF uses a boundary 
...
zone only. Consider, however, that they are full domain fields that were 
...
\end{verbatim}
\subsubsection{Add calls to \textsl{da\_transfer\_xatowrftl\_adj\_lbc} and \textsl{da\_transform\_vtox\_adj}}
\begin{verbatim}
...
\end{verbatim}
\subsubsection{Add the adjoint of converting WRFTL variables to analysis increments at boundaries}
\begin{verbatim}
...
\label{sec:single}
Analysis increments due to a single observation produced by a data assimilation system implicitly provide the effective background error covariance matrix $\mathbf{B}$ and describe how the tangent linear and adjoint model propagate the observational information, see \cite{Huang2009}. After any new capability for WRF 4D-Var system was developed, a single observation experiment is an effective and clean way to show the impact of the new capability on the analysis.
As in Fig. 3 of \cite{Huang2009}, given a single temperature observation at the end of the time window of 4D-Var, we knew that the 4D-Var increments have a temporal dimension, the increments at 6 h give a graphic representation of the background error covariance matrix at 6 h, $\mathbf{MBM}^T$, where $\mathbf{M}$ and $\mathbf{M}^T$ are the WRF tangent linear model and adjoint model respectively. For current WRF 4D-Var version, WRF is a regional model, and both initial condition and boundary condition determine the future trajectory of the model. Therefore, the observations close to or within the lateral boundary relaxation region should not only impact the initial condition but also the boundary condition.
To illustrate and verify the correctness of the new capability of adding lateral boundary condition (LBC) into the control variable in 4D-Var, we design 2 single observation experiments similar to that in paper \cite{Huang2009}: 
...
We moved the single temperature observation at 0600 UTC to
$28\,^{\circ}\mathrm{N}$, $75\,^{\circ}\mathrm{W}$, as the red cross in
Fig.~\ref{fig:boundary_0h}. Fig.~\ref{fig:boundary_0h} show the analysis
increments due to the 6 h temperature observation with and without LBC control.
Because the observation is within the lateral boundary relaxation region, the
upstream response of 6 h before should be outside the domain, which means the
analysis response at 0 h should be in the tendency of boundary condition. This
is the case in Fig.~\ref{fig:boundary_0h}(b), there is no significant response
in initial condition when the LBC control was turned on. However, if the LBC
control was turned off,  4D-Var has to produce some unrealistic responses in initial condition to fit the observation, as shown in Fig.~\ref{fig:boundary_0h}(a).  Then, we integrated the increments forward from 0 hour to 6 hour. The perturbation development in Fig.~\ref{fig:boundary_6h} shows that the original perturbation along south boundary almost doesn't change during the 6 hours, the positive perturbation was developed locally, which means there is no/very few perturbation propagated from LBC tendency. However, in Fig.~\ref{fig:boundary_lbc_6h}, the perturbation along south boundary was developed from scratch and became stronger and stronger, apparently, the LBC tendency did the job. Finally, comparing the increments at 6 h in both figures, we found that the increment in experiment with LBC control fit the observation location much better than that that in the experiment without LBC control. All the results confirm the assumption that the LBC control has significant impact on the assimilation of the observation which close to the LBC during the 4D-Var time window.
\begin{figure}[t]
\subfigure[Without LBC control]{
...
The above two experiments also confirm the correctness of implementation of the LBC control in 4D-Var.   

\section{Real data experiment}

We randomly selected a period, 1-5 April 2008, for the real data experiments over the CONUS domain. The domain size is $43\times31\times28$, grid distance is $135km$. We run the 4D-Var as well as the subsequent 24 hours forecast every 12 hours.
...
From the discussion in section \ref{sec:single}, we know that the 4D-Var could produce unrealistic  analysis increments to fit the downstream observations close to boundary relaxation region if LBC were not included as control variables. 
%Additionally including LBC in 4D-Var will reduce the fake or overestimated analysis increments in IC, because some of the analysis response due to the observations close to boundary will be redirected to LBC increments.  
The IC analysis increments combining with the LBC increments will produce a better observation fitting. In general, we could expect that the experiments with LBC control should end with a smaller $J_b$ and $J_o$ than which in experiments without LBC control. 
%Note: This expectation should only be treated as a high probability phenomenon, it is not a definite conclusion since the $J_b$ and $J_o$ are the mixed and together index of the changes of the background.  
Table~\ref{t1} list the final $J_b$ and $J_o$ in each experiments. The red numbers in Table~\ref{t1} are two exceptions , all other cases confirm our expectation. Especially, all $J_o$ with LBC control become smaller, which means that during the 6-hour time window, the model trajectory fits observations better than that in experiment without LBC control. 
\begin{table}[t]
...
Next, we will show the verification scores of the 12 and 24 hour forecast upon the observations. Figs.~\ref{fig:bias_12h}, ~\ref{fig:rmse_12h} and Figs.~\ref{fig:bias_24h}, ~\ref{fig:rmse_24h} are the BIAS and RMSE profiles of the 12 and 24 hours forecast verified against the observations respectively. Basically, the verification results are quite similar with or without LBC control. According to the personal communication with the developers of HIRLAM system which has similar formulation of the LBC control, the forecast score improvement due to the LBC control is difficult to be shown over by pre-operational parallel experiments. Sensitive cases may need to be accumulated to document the impact of the LBC control.

\begin{figure}[t]
\begin{center}
\noindent\includegraphics[width=39pc, viewport=20 250 580 750, clip]{./figures/Profile_BIAS_12}
\end{center}
...
\begin{figure}[t]
\begin{center}
\noindent\includegraphics[width=39pc, viewport=20 250 580 750, clip]{./figures/Profile_RMSE_12}
\end{center}
...
\begin{figure}[t]
\begin{center}
\noindent\includegraphics[width=39pc, viewport=20 250 580 750, clip]{./figures/Profile_BIAS_24}
\end{center}
...
\begin{figure}[t]
\begin{center}
\noindent\includegraphics[width=39pc, viewport=20 250 580 750, clip]{./figures/Profile_RMSE_24}
\end{center}
...
We noticed minor degradations on the $v$ scores and will continue the work to
refine the experiment configuration. The current WRF/WRFDA setup has a boundary
updating procedure between data assimilation and the subsequent forecast to
account for the initial condition changes due to data assimilation. The update
only applies on the boundary tendencies from 0 to 6 hour. Once the LBC of the
time window was included in the 4D-Var minimization, the tendency of the 0--6 hour LBC  will be updated automatically after the analysis, which also means the state at the end of the time window will be changed too. Then, not only the 0--6 hour LBC tendency should be updated, the 6--12 hour LBC tendency should be updated, this capability should be developed.
