...
Next, we will show the verification scores of the 12 and 24 hour forecast upon the observations. Fig.~\ref{fig:bias_12h} and Fig.~\ref{fig:bias_24h} are the bias profiles of the 12 and 24 hours forecast verify against the observations. Basically, the verification results are quite mixed, it is hard to decide which experiment is better than other one since the period of experiments is very limited. According to the personal communication with the developers of HIRLAM system, the improvement due to the LBC control is very trivial and unnoticeable even in a long time operational parallel experiments. Therefore, long time parallel run is needed to evaluate the impact of the LBC control on the forecast.
\begin{figure}[t]
\begin{center}
