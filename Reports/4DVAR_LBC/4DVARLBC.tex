...
Again following \cite{Kawa2007} we will introduce a cost function term $J_{lbc}$ that will measure the distance to the un-perturbed lateral boundary conditions used in the non-linear model 
\begin{equation}
J_{lbc}=(\delta{\mathbf{x}_{lbc}}(t_k))^T\bm{\mathsf{B}}_{lbc}^{-1}\delta{\mathbf{x}_{lbc}}(t_k)
\end{equation}
where $\bm{\mathsf{B}}_{lbc}$ represents the covariance of the lateral boundary condition 
...
conditions, we will simply apply $\bm{\mathsf{B}}_{lbc}=\bm{\mathsf{B}}$. 
...
conditions 
\begin{displaymath}
(\frac{\partial{\delta{\mathbf{x}_{lbc}}}}{\partial{t}})=\frac{\delta{\mathbf{x}_{lbc}}(t_k)-\delta{\mathbf{x}_{lbc}}(t_0)} {t_{k}-t_0}
\end{displaymath}
...
is any model state variable and $\mathbf{x}_{lbc}$ is the corresponding boundary value 
provided by the host model. $\mathbf{x}_{lbc}$ is speciﬁed in the following form:
\begin{equation}
\mathbf{x}_{lbc}(time=t)=\mathbf{x}_{lbc}(time=t_0)+(t-t_0)\frac{\partial{\mathbf{x}_{lbc}}}{\partial{t}}
\end{equation}
Considering a data assimilation window from time $t_0$ until time $t_k$ and 
having $\delta{\mathbf{x}}(t_0)$ and $\delta{\mathbf{x}_{lbc}}(t_k)$ as the assimilation control variables, the quantities needed for the LBC of the tangent linear WRF model are given by
\begin{equation}
...
\end{equation}
The lateral boundary conditions for the adjoint model, $\mathbf{x}_{lbc}^{AD}(t_0)$ and $(\frac {\partial{\mathbf{x}}_{lbc}} {\partial{t}})^{AD}$
...
Note that $\mathbf{x}_{lbc}^{AD}(t_0)$ and $(\frac{\partial{\mathbf{x}_{lbc}}}{\partial{t}})^{AD}$ will be defined in the boundary relaxtion 
...
Also note that the calculation of $J_{lbc}$ and $\frac{\partial{J_{lbc}}} {\partial{\mathbf{v}_{lbc}}}$, where $\mathbf{v}_{lbc}$ is the lateral boundary condition control variable in control vector space, will follow exactly the same calculations as for the background error constraint. 
...
\end{verbatim}
\subsubsection{Change cv\_size initialization due to $J_{lbc}$}
\begin{verbatim}
